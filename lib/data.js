...
export async function getStaffScheduleByDay(supabase, year, dayNumber) {
  ...
  const rows = (data || []).map((row) => ({
    ...row,
    activity_name:
      String(row.activity_name || "").trim() === "Counselor Induction Ceremony"
        ? "Pass the TORCH"
        : row.activity_name,
  }));
...
export async function getPinnedUrgentAnnouncements(supabase, year, limit = 4) {
  const { data, error } = await supabase
    .from("announcements")
    .select("id, title, body, message_type, created_at")
    .eq("program_year", year)
    .eq("is_pinned", true)
    .in("message_type", ["urgent", "schedule_change"])
    .order("created_at", { ascending: false })
    .limit(limit);
...
export async function getResourceCategories(supabase, year) {
  const { data, error } = await supabase
    ...
    .select(
      "id, name, icon, sort_order, resource_items(id, title, body, url, image_url, visibility, sort_order, updated_at)",
    )
    .eq("program_year", year)
    .order("sort_order", { ascending: true });
...
export async function getGuildPreferenceBoardData(supabase, year) {
  ...
  const rows = students.map((student) => {
    ...
    return {
      ...
      rank_1_id: preference?.rank_1 ?? null,
      rank_2_id: preference?.rank_2 ?? null,
      rank_3_id: preference?.rank_3 ?? null,
...
export async function getUserProfiles(supabase, year = null) {
  let query = supabase
    .from("user_profiles")
    .select("id, full_name, email, role, program_year, team_key, guild_id, room_number, phone_number, is_active, created_at, pronouns, specialty_tag, cotl_color, superpower, show_in_directory")
